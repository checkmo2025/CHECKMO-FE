...
const MyStoryPage = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any, _vars, ctx) => {
      ...
      if (status === 400) {
        setErrorModal({ open: true, message: "잘못된 요청입니다. 다시 시도해주세요." });
      } else if (status === 403) {
        setErrorModal({ open: true, message: "삭제 권한이 없습니다." });
      } else if (status === 404) {
        setErrorModal({ open: true, message: "책 이야기를 찾을 수 없습니다." });
      }
    },
    onSettled: () => {
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any, _vars, ctx) => {
      ...
      if (status === 400) {
        ...
        setErrorModal({ open: true, message: "수정 권한이 없습니다." });
      } else if (status === 404) {
        setErrorModal({ open: true, message: "책 이야기를 찾을 수 없습니다." });
      }
    },
